...
export default function OnboardingStepFijos({ value, onChange }) {
  ...
  // Reemplaza el input de 'Hora fin' por un input/desplegable de duración
  ...
  const agregar = () => {
    ...
    setBloques([...bloques, { ...nuevo, hora_fin, es_inflexible: true, bloque_tipo: "fijo" }]);
    setNuevo({ nombre_actividad: "", dias: [], hora_inicio: "", duracion: 60, ubicacion: "", requiere_recordatorio: false });
  };
  ...
  const guardar = () => {
    ...
    if (
      ...
      const end = new Date(start.getTime() + Number(nuevo.duracion) * 60000);
      const hora_fin = end.toTimeString().slice(0,5);
      nuevosBloques = [...bloques, { ...nuevo, hora_fin, es_inflexible: true, bloque_tipo: "fijo" }];
  ...
  return (
    <div className="flex justify-center items-center min-h-[60vh]">
      <Card className="w-full max-w-xl shadow-glow">
        <CardHeader>
          <CardTitle className="text-lg gradient-text">Bloques fijos e inflexibles</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            {bloques.length > 0 && (
              <div className="mb-4">
                <div className="font-medium mb-2 text-muted-foreground">Tus bloques fijos:</div>
                    ...
                    <li key={i} className="flex items-center justify-between bg-muted/50 rounded-lg px-3 py-2">
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          ...
                          {b.requiere_recordatorio && <Badge className="bg-blue-500 text-white ml-2">Recordatorio</Badge>}
                        </div>
                      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input placeholder="Ej: Trabajo, Clases..." value={nuevo.nombre_actividad} onChange={e => setNuevo(n => ({ ...n, nombre_actividad: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label>Ubicación</Label>
                <Input placeholder="Opcional" value={nuevo.ubicacion} onChange={e => setNuevo(n => ({ ...n, ubicacion: e.target.value }))} />
              </div>
              <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 mt-2">
              <input type="checkbox" id="recordatorio" checked={nuevo.requiere_recordatorio} onChange={e => setNuevo(n => ({ ...n, requiere_recordatorio: e.target.checked }))} />
              <Label htmlFor="recordatorio" className="text-sm">Recordatorio</Label>
            </div>
            <div className="flex gap-3 mt-4">
              <Button type="button" onClick={agregar} className="shadow-glow">Agregar bloque fijo</Button>
